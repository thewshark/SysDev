...
union all select  Descricao as 'Description', Documento as 'Code', 0 as 'Orders', 0 as 'Sales', 0 as 'Purchase', 1 as 'Internal', 0 as 'Stock', 0 as 'Transfer', CASE WHEN CLIENTES = 1 THEN 'CL' WHEN FORNECEDORES = '1' THEN 'FL' ELSE '' END as 'Entity',1 as 'ValidaStock', 0 AS 'StockBreak','' as DefaultEntity  FROM DocumentosInternos DI WITH(NOLOCK)
union all select  Descricao as 'Description', Documento as 'Code', 0 as 'Orders', 0 as 'Sales', 0 as 'Purchase', 0 as 'Internal', 0 as 'Stock', 1 as 'Transfer', CASE WHEN BensCirculacao=1 and TipoDocVenda<>'' THEN 'CL' ELSE '' END as 'Entity',1 as 'ValidaStock', 0 AS 'StockBreak','' as DefaultEntity  from INV_DOCUMENTOSTRANSFERENCIA DT WITH(NOLOCK) WHERE DT.TIPOTRANSFERENCIA = 0
GO
...
left join Artigo art WITH(NOLOCK) on art.Artigo = cb.Artigo
...
, al.Activo as Actif
from ArtigoLote al WITH(NOLOCK)
join Artigo a WITH(NOLOCK) on al.artigo=a.artigo
...
from INV_NumerosSerie ans WITH(NOLOCK)
...
'' as Location,
'' as InternalStampDoc
from INV_CabecInventarios cab WITH(NOLOCK)
left join u_Kapps_StockLines WITH(NOLOCK) on u_Kapps_StockLines.OrigStampHeader = cab.ID and u_Kapps_StockLines.Status = 'A' and u_Kapps_StockLines.Syncr = 'N' 
...
'' as 'Filter1','' as 'Filter2','' as 'Filter3','' as 'Filter4','' as 'Filter5'
FROM INV_LinhasInventarios lin WITH(NOLOCK) 
JOIN INV_CabecInventarios cab WITH(NOLOCK) on cab.Id = lin.IdCabecInventarios
JOIN Artigo Art WITH(NOLOCK) ON Art.Artigo = lin.Artigo 
LEFT JOIN ArtigoUnidades ARTUND WITH(NOLOCK) on ARTUND.Artigo = lin.Artigo AND ARTUND.UnidadeOrigem = lin.Unidade AND ARTUND.UnidadeDestino = Art.UnidadeBase
LEFT JOIN INV_DetalhesLinhasInventarios LinDet WITH(NOLOCK) ON lin.Id=LinDet.IdLinhaInventario
...
--
,'' as Barcode
from INV_CabecTransferencias cab WITH(NOLOCK)
left join Clientes cli WITH(NOLOCK) ON cli.Cliente = cab.Entidade
left join INV_DocumentosTransferencia tdoc WITH(NOLOCK) on tdoc.Documento = cab.TipoDoc
...
(SELECT COALESCE(ABS(SUM(Ltr.QuantTrans)),0) 
...
(select COALESCE(sum(u_Kapps_DossierLin.Qty2),0) from u_Kapps_DossierLin WITH(NOLOCK) where u_Kapps_DossierLin.Status = 'A' and u_Kapps_DossierLin.Integrada = 'N' and (cast(lin.IdCabecTransferencias as varchar(255))=u_Kapps_DossierLin.stampbo) and (CAST(lin.ID AS VARCHAR(255)) + '*' + CAST(lin.numlinha as varchar(15)) = u_Kapps_DossierLin.Stampbi))) 
as 'QuantityPending',

(select COALESCE(sum(u_Kapps_DossierLin.Qty2),0) from u_Kapps_DossierLin WITH(NOLOCK) where u_Kapps_DossierLin.Status = 'A' and u_Kapps_DossierLin.Integrada = 'N'  and (cast(lin.IdCabecTransferencias as varchar(255))=u_Kapps_DossierLin.stampbo) and (CAST(lin.ID AS VARCHAR(255)) + '*' + CAST(lin.numlinha as varchar(15)) = u_Kapps_DossierLin.Stampbi)) as 'QuantityPicked', 
Art.UnidadeBase AS 'BaseUnit',
lin.Unidade as 'BusyUnit',
...
, '' as PalletType
, 0 as PalletMaxUnits
FROM INV_LinhasTransferencias lin WITH(NOLOCK) 
left join INV_CabecTransferencias cab WITH(NOLOCK) on cab.Id = lin.IdCabecTransferencias
left join Artigo Art WITH(NOLOCK) on Art.Artigo = lin.Artigo 
left join ArtigoUnidades ARTUND WITH(NOLOCK) on ARTUND.Artigo = lin.Artigo AND ARTUND.UnidadeOrigem = lin.Unidade AND ARTUND.UnidadeDestino = Art.UnidadeBase
left join INV_DocumentosTransferencia tdoc WITH(NOLOCK) on tdoc.Documento = cab.TipoDoc
where 
